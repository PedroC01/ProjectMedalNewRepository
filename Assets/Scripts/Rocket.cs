...
public class Rocket : MonoBehaviour
{
    ...
    public float timerToCollidde=0.2f;
    ...
    void Update()
    {
        timerToCollidde = Mathf.Clamp(timerToCollidde - Time.deltaTime, 0, 2);
        if (timerToCollidde >= 0) canCollide = true;
        if (thisGotShooted)
        {
            if (isFollowing)
            {
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (Medapart != null)
        {
            Medapart.ApplyDamage(RocketDamage);
            ...
            if (Medapart.playerX == 2 || Medapart.playerX == 1)
            {
                ...
                foreach (Collider coll in colliders)
                {
                    var medaPart = coll.GetComponent<MedaPartScript>();
                    if (medaPart != null)
                    {
                        medaPart.ApplyDamage(RocketDamage);
